...
import { DeliveryRepositoryPort, CreateDeliveryData } from '../../../domain/ports/outbound/delivery.repository.port.js';
...
export class DeliveryPrismaRepository implements DeliveryRepositoryPort {
  ...
  async create(data: CreateDeliveryData): Promise<Result<Delivery>> {
    try {
      ...
      return Result.ok(
        new Delivery(d.id, d.transactionId, d.customerId, d.address, d.city, d.department, d.postalCode, d.status, d.createdAt),
      );
    } catch (error) {
  ...
  async findByTransactionId(transactionId: string): Promise<Result<Delivery | null>> {
    try {
      const d = await this.prisma.delivery.findUnique({ where: { transactionId } });
