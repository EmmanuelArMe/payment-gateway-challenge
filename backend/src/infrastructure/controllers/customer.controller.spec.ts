...
describe('CustomerController', () => {
  ...
  const customer = new Customer('cust-1', 'John Doe', 'john@example.com', '3001234567', new Date());
  ...
  it('returns a customer by id', async () => {
    ...
    await expect(controller.findOne('cust-1')).resolves.toEqual({ data: customer });
    expect(getCustomerUseCase.execute).toHaveBeenCalledWith('cust-1');
  });

  it('throws not found when customer lookup fails', async () => {
    getCustomerUseCase.execute.mockResolvedValue(Result.fail('Customer not found'));
  ...
  it('returns a customer by email', async () => {
    ...
    await expect(controller.findByEmail('john@example.com')).resolves.toEqual({ data: customer });
    expect(getCustomerByEmailUseCase.execute).toHaveBeenCalledWith('john@example.com');
  });

  it('throws not found when customer email lookup fails', async () => {
    getCustomerByEmailUseCase.execute.mockResolvedValue(Result.fail('Customer not found'));

    await expect(controller.findByEmail('missing@example.com')).rejects.toMatchObject<HttpException>({
      message: 'Customer not found',
      status: HttpStatus.NOT_FOUND,
    });
  });
